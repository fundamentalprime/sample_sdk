//+------------------------------------------------------------------+
//|                                        FundamentalPrime_Base.mq5 |
//|                                     Fundamental Prime , SIA Inc. |
//|                                      https://www.funda-prime.com |
//+------------------------------------------------------------------+
//
// 当ソースファイルは MT5のMQL5を利用したプログラミングを使い、Fundamental Prime Web API へ
...
#include <FundamentalPrime1140_mt5.mqh>
...
void OnTick()
  {
   ...
   if(mql4_TickVolume(_Symbol,_Period,0) > 1)
      return;
...
//サーバータイムを計算
   datetime targetDatetime = (datetime)MathFloor((long)mql4_Time(_Symbol,_Period,0) / (60 * p)) * (60 * p);
