...
input string FPUSERID = "";
string FPEAID = "";

// サーバー関係
string CURRENT_USERAGENT = "Fundamental Prime MT5:" + IntegerToString(TerminalInfoInteger(TERMINAL_BUILD)) + ":" + FPEAID + "@" + FPUSERID;
...
// 内部 状況保持変数
string api_last_date_or_datetime; // FP_WEBAPI_REQUEST の accessPerDayForBacktest で利用 (MQLInfoInteger(MQL_TESTER) == 1) ? true : false時 最後に呼び出した日時を保持（重複データ呼び出ししないように制御)
...
// 他のユーザーに配布する場合は FPEAIDは必須　
   ...
   */
   ...
   if(!(MQLInfoInteger(MQL_DLLS_ALLOWED) == 1) ? true : false)
     {
      ...
      if(FileIsExist(stringURL,fileCOMMON))
        {
         ...
         while(!FileIsEnding(h))
           {
            strData += FileReadString(h,0) + CharToString(0x0a);
           }
         FileClose(h);
...
// パラメータ t　のための日時文字列組み立て YYYYMMDDHHNNSS
   ...
   if(dt == NULL)
     {
      stringDT = TimeToString(DateConvertServer2API(mql4_Time(_Symbol,_Period,0)),TIME_DATE | TIME_SECONDS);
     ...
     {
      stringDT = TimeToString(DateConvertServer2API(dt),TIME_DATE | TIME_SECONDS);
      StringReplace(stringDT,":","");
      StringReplace(stringDT,".","");
      StringReplace(stringDT," ","");
     }

   if((MQLInfoInteger(MQL_TESTER) == 1) ? true : false)
     {
   ...
   if(!(MQLInfoInteger(MQL_TESTER) == 1) ? true : false)
     {
      ...
      //mt4 stringURL += "&seq=" + IntegerToString(TimeMinute(TimeLocal())) + IntegerToString(TimeSeconds(TimeLocal()));
      string seqString= TimeToString(TimeLocal(),TIME_SECONDS);
      StringReplace(seqString,":","");
      stringURL += "&seq=" + seqString;
     }
...
//
   bool hasCache = false;
   if((MQLInfoInteger(MQL_TESTER) == 1) ? true : false && PROGRAMMED_CACHE_ENABLE)
     {
      apiReturnString = "";
   ...
   if((MQLInfoInteger(MQL_TESTER) == 1) ? true : false && PROGRAMMED_CACHE_ENABLE && !hasCache)
     {
      //
      // キャッシュ保存処理
      //
...
//3月第2日曜日午前2時〜11月第1日曜日午前2時
   ...
   TimeToStruct(targetDatetime,dt_struct);

// mt4  switch(TimeYear(targetDatetime))
   switch(dt_struct.year)
     {
      case 2015:
...
double GetPipsByPoint(string Currency)
  {
//mt4 double Symbol_Digits = MarketInfo(Currency,MODE_DIGITS);
   double Symbol_Digits = (double)SymbolInfoInteger(Currency, SYMBOL_DIGITS);
...
// 利用シーンに合わせて変更してください
// 基本的な考え方は、(MQLInfoInteger(MQL_TESTER) == 1) ? true : false中は終了を促し不用意なアクセス数を防ぎます。
...
//|                                                                  |
//+------------------------------------------------------------------+
datetime mql4_Time(string symbol, ENUM_TIMEFRAMES tf,int shift)
  {
   datetime Time[1];
   int count= TerminalInfoInteger(TERMINAL_MAXBARS);
   if(CopyTime(symbol,tf,shift,1,Time) == 1)
      return Time[0];
   return -1;
...
//|                                                                  |
//+------------------------------------------------------------------+
long mql4_TickVolume(string symbol, ENUM_TIMEFRAMES tf,  int shift)
  {
   long Volume[1];
   if(CopyTickVolume(symbol,tf,shift,1,Volume)==1)
      return Volume[0];
   return -1;
  }
//+------------------------------------------------------------------+
